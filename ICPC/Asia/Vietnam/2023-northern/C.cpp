...
map<vector<int>, mint> cache[111];
int n;
mint dp(int i, vector<int>& dists, const vector<bool>& can_use) {
    if (i == n) return mint(1);
    if (cache[i].count(dists)) return cache[i][dists];

    mint res = 0;
    for (int cur = 0; cur < 8; ++cur)
        if (can_use[cur] && dists[cur] == cur) {
            vector<int> new_dists(dists.size());
            for (int x = 0; x < 8; ++x) new_dists[x] = min(dists[x] + 1, x);
            new_dists[cur] = 0;

            res += dp(i+1, new_dists, can_use);
        }
    return cache[i][dists] = res;
}

void solve() {
    int k; cin >> n >> k;
    vector<bool> can_use(8, false);
    while (k--) {
        int x; cin >> x;
        can_use[x-1] = true;
    }
    vector<int> dists(8);
    std::iota(dists.begin(), dists.end(), 0);

    cout << dp(0, dists, can_use) << endl;
}
