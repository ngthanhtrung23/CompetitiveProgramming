...
int32_t main() {
    ...
    while (cin >> n >> x) {
        ...
        Matrix<modular> trans_fibo({
            {1, 1},
            {1, 0},
        });

        if (left == 0) {  // x == n
            sum = trans_fibo.pow(right + 1)[0][0];
        } else {
            ...
            Matrix<modular> first_bad({
                {0},
                {0},
                {1},
            });
            Matrix<modular> first_good({
                {1},
                {1},
                {0},
            });

            auto res_bad = trans.pow(left - 1) * first_bad;
            auto res_good = trans.pow(left - 1) * first_good;

            if (right == 0) {
                sum += res_good[0][0] + res_good[1][0] + res_good[2][0];
                sum += res_bad[0][0] + res_bad[2][0];
            } else {
                sum += (res_good[0][0] + res_good[2][0]) * (trans_fibo.pow(right + 1))[0][0];
                sum += res_good[1][0] * (trans_fibo.pow(right))[0][0];

                sum += (res_bad[0][0] + res_bad[2][0]) * trans_fibo.pow(right)[0][0];
                sum += res_bad[1][0] * trans_fibo.pow(right - 1)[0][0];
            }
        }
