...
void solve() {
    int n; cin >> n;
    vector<int> orig(n);
    vector<vector<int>> ids_of(1011);
    for (int i = 0; i < n; ++i) {
        cin >> orig[i];
        ids_of[orig[i]+500].push_back(i);
    }

    vector<int> a(n, -1000111000);
    auto max_sub_arr = [] (const vector<int>& a) -> int {
        int res = -1000111000;
        int cur_sum = 0;
    ...
    for (int maxVal = -500; maxVal <= 500; ++maxVal) {
        if (ids_of[maxVal + 500].empty()) continue;
        for (int id : ids_of[maxVal + 500]) a[id] = orig[id];

        res = max(res, max_sub_arr(a) - maxVal);
    }
    cout << res << endl;
}
