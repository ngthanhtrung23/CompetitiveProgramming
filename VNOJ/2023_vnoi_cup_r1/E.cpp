...
void solve() {
    int boardSize, nBombs, destroy;
    cin >> boardSize >> nBombs >> destroy;
    vector<pair<int,int>> bombs(nBombs);
    for (auto& [x, y] : bombs) {
        cin >> x >> y;
    }
    sort(bombs.begin(), bombs.end());

    auto canUp = [&] (int x, int y) {
        return (x <= destroy || y >= boardSize - destroy);
    };
    auto canDown = [&] (int x, int y) {
        return (x >= boardSize - destroy || y <= destroy);
    };
...
#ifndef RR
    if (nBombs <= 2000) {
        REP(i,nBombs) FOR(j,i+1,nBombs) {
            auto [x, y] = bombs[i];
            int l = x - destroy, r = x + destroy;
            int u = y - destroy, d = y + destroy;

            if (l <= bombs[j].first && bombs[j].first <= r
                    && u <= bombs[j].second && bombs[j].second <= d) {
                dsu.merge(i, j);
            }
        }
    }
    ...
    REP(i,nBombs) {
        int start = bombs[i].second - destroy;
        while (!prevs.empty()) {
            auto it = prevs.lower_bound(make_pair(start, -1));

            // nothing to connect
            if (it == prevs.end() || it->first > bombs[i].second + destroy) break;

            int j = it->second;
            // invalid
            if (bombs[j].first < bombs[i].first - destroy) {
                prevs.erase(it);
                continue;
            }

            dsu.merge(i, j);
            start = bombs[j].second + destroy;
        }

        prevs.insert(make_pair(bombs[i].second, i));
    }

    vector<vector<int>> comps(nBombs);
    REP(i,nBombs) {
        int r = dsu.getRoot(i);
        comps[r].push_back(i);
    }
    ...
    for (const auto& comp : comps) {
        ...
        for (int u : comp) {
            auto [x, y] = bombs[u];
            if (canUp(x, y)) hasUp = true;
            if (canDown(x, y)) hasDown = true;
        }
        if (hasUp && hasDown) {
            for (int u : comp) {
                all.push_back(bombs[u]);
            }
        }
    }
