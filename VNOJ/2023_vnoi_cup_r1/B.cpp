...
double go(double vr, double vs) {
    double l = 0.0, r = 100.0, res = 100.0;
    REP(turn,100) {
        double mid = (l + r) / 2.0;
        double dist = mid * vr + min(10.0, mid * vs);
        if (dist >= 100.0) {
            res = mid;
            r = mid;
        } else l = mid;
...
void solve() {
    int ntest; cin >> ntest;
    while (ntest--) {
        double vr1, vs1, vr2, vs2;
        cin >> vr1 >> vs1 >> vr2 >> vs2;

        double t1 = go(vr1, vs1);
        double t2 = go(vr2, vs2);

        if (abs(t1 - t2) < 1e-9) cout << "Draw";
        else if (t1 < t2) cout << "Megumin";
        else cout << "Yunyun";
        cout << '\n';
    }
}
