...
int32_t main() {
    ...
    modular p, q, r; cin >> p >> q >> r;

    Matrix<modular> trans(n + 3, n + 3);
    // row 0
    REP(i,n) trans[0][i] = c[i+1];
    trans[0][n] = p;
    trans[0][n+1] = q;
    trans[0][n+2] = r;
    ...
    // row n -> n+2: 1 (n+1) (n+1)^2
    trans[n][n] = 1;
    trans[n+1][n] = trans[n+1][n+1] = 1;
    trans[n+2][n] = 1; trans[n+2][n+1] = 2; trans[n+2][n+2] = 1;
    ...
    modular res = 0;
    REP(i,n) res += trans[n-1][i] * a[n-1-i];
    res += trans[n-1][n] + trans[n-1][n+1] * n + trans[n-1][n+2] * (n*n);
    cout << res << endl;
