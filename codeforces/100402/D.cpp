...
int n, k;
vector<int> a[22];
BigInt s2[111][111], bell[111];

void init() {
    FOR(n,0,100) FOR(k,0,100) {
        if (n == 0 && k == 0) s2[n][k] = 1;
        else if (n == 0 || k == 0) s2[n][k] = 0;
        else {
            s2[n][k] = s2[n-1][k] * k + s2[n-1][k-1];
        }
        bell[n] += s2[n][k];
    }
}
...
int main() {
    ...
    while (cin >> n >> k) {
        ...
        REP(mask,1<<k) {
            ...
            int cnt = 0;
            FOR(i,1,n) if (dsu.lab[i] < 0) ++cnt;

            if (__builtin_popcount(mask) % 2) res -= bell[cnt];
            else res += bell[cnt];
        }
        cout << res << endl;
    }
