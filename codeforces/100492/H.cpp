...
void generate(int i, int lim, BigInt value, int isQuery)
{
  if (i == lim) 
  {
    if (!isQuery) s.insert(value);
    else
    {
      if (value > target)
        return;
      auto u = s.upper_bound(target / value);
      if (u != s.begin())
      {
        u--;
        value *= *u;
  ...
  {
    generate(i + 1, lim, value, isQuery);
    generate(i + 1, lim, value * p[i], isQuery);
  }
}
...
int main()
{
  ...
  while (cin >> n, n)
  {
    ...
    s.clear();
    ans = BigInt(1);
    generate(0, n / 2, BigInt(1), 0);
    generate(n / 2, n, BigInt(1), 1);
