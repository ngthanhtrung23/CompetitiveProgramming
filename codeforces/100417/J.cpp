...
BigInt divideMod(BigInt a, BigInt b, BigInt n) {
    a = a % n;
    if (a < 0) a += n;
...
int main() {
    ...
    while (cin >> n >> x >> y) {
        try {
            BigInt t = divideMod(y - x + 2, 3, n) - 1;

            BigInt u = divideMod(x + y, t + 1, n) - 1;
            BigInt m = divideMod(u, 2, n);

            if (m * m * m % n != x) throw 1;
            if ((m+1) * (m+1) * (m+1) % n != y) throw 1;
