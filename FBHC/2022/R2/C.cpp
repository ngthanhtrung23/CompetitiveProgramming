...
void solve() {
    vector<mint> gt(10111000);
    gt[0] = 1;
    FOR(i,1,SZ(gt) - 1) gt[i] = gt[i-1] * i;

    auto order = [&] (int n, int k) -> mint{
    ...
    FOR(test,1,ntest) {
        ...
        mint equal_or_less = order(cx + c0 + c1, k+1);
        mint less = order(c0, k+1);
        mint at_least_one_equal = equal_or_less - less;

        mint cond = mint(cx) / mint(cx + c1);

        mint res = at_least_one_equal * cond;
        mint mau = order(call, k+1);

        cout << "Case #" << test << ": " << res / mau << endl;
    }
}
